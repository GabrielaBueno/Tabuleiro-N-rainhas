...
/*Uma função para verificar se uma rainha pode
   ...
   já colocado em colunas de 0 a col 7.
...
/* Uma função de utilidade recursiva para resolver N
   Problema da rainha */
bool resolveNRUtil(int **mat, int col, int nu,int p, int *back) 
{ 
     int N = nu;
     int pp=0;
      pp=p;
    ...
    { 
        ...
        { 
            ...
            //cont= contaQueen(mat, i, col, N);
        
            if ( resolveNRUtil(mat, col + 1, N,pp,back) )     //recorrer para colocar o resto das rainhas
                return true; 
...
/* Esta função resolve o problema N Queen usando
   ...
   soluções viáveis. */
bool resolveNR(int **matz, int n, int p, int *back) 
{ 
    int **mat = matz; 
    int nu=n;
    int P=p;
    liberaQueen(mat,nu);//libera o tabuleiro para troca de posição
    if ( resolveNRUtil(mat, 0, nu, P,back) == false ) // condicao para verificar se a solução do tabuleiro informado é possivel
    { 
...
void preencheMat(int **mat,int n){
    ...
    for ( i = 0; i < n; i++){ // percorre todas as colunas da matriz
        r=rand()%n;  //seta o valor randomico e atribui a variavel r;             
...
int somaAtaque(int **mat,int n,int row, int col ){
  ...
  for (i = 0; i < N; i++){
      for(j = 0; j<N; j++) 
        ...
        if (mat[i][j]==2 || mat[j][i]==2 || mat[i][i]==2 || mat[j][j]==2)
...
{ 
    ...
    int p=0; //variavel para armazenar opção de passo-a-passo
    int ataq=0; //variavel pra armazenar numero de ataques
    int *back = NULL;
    int backt = 0;
    back = &backt;
    ...
    printf("\n");
    printf("\n Deseja exibir na tela o passo-a-passo do backtrack? (digite 1 para sim) : ");
    scanf("%d",&p); // le do usuario se imprimi passo a passo 1 ou 0;
    printf("\n");
    ...
    printf("Tabuleiro inicial %d x %d casas: \n\n", n,n);  
    imprimeSol(mat,n); //função para imprimir tabuleiro aleatorio inicial com n rainhas 
    ataq=somaAtaque(mat,n,l,c);
    printf("Numero de ataques tabuleiro inicial: %d",ataq);
    printf("\n\nTabuleiro final com a solucao para %d rainhas que nao se atacam. \n\n",n);
    resolveNR(mat,n,p,back); //função para encontrar e imprimir a solução do N-rainhas
    printf("Numero de ataques tabuleiro final: %d",0);
    printf("\nNumero de backtrack: %d",*back);
